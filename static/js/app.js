...
async function submitWord() {
    ...
    try {
        ...
        btn.style.color = ''; // Ensure text is visible again just in case
    }
}
...
function playSubmitEnvelopeAnimation(btn) {
    return new Promise(resolve => {
        ...
        // 1. Hide placeholders, clear fields, and capture layout
        fields.forEach(id => {
            const el = document.getElementById(id);
            if (el) {
                cachedPlaceholders[id] = el.getAttribute('placeholder') || '';
                el.setAttribute('placeholder', ''); // Clear placeholder
        ...
        // Hide button text to make it look like an empty mailbox base
...
const CHALLENGE_COMMENTS_PER_PAGE = 10;
let challengeExpanded = false;
let activeChallengeView = null;
let currentChallengeId = null;
let currentChallengeCommentPage = 1;

function setupChallengeBox() {
...
function toggleChallengeBox() {
    ...
    body.style.display = challengeExpanded ? 'block' : 'none';
    if (icon) icon.innerHTML = challengeExpanded ? '&#9650;' : '&#9660;';
...
function openChallengeDiscussion(challengeId, foreignWord, meaning) {
    ...
    activeChallengeView = view;
    loadChallengeComments(challengeId, 1);
}

function closeChallengeDiscussion() {
    if (!activeChallengeView) return;

    const backdrop = document.getElementById('modalBackdrop');
    backdrop.classList.remove('show');
    activeChallengeView.classList.remove('expanded');

    setTimeout(() => {
        if (activeChallengeView) activeChallengeView.remove();
...
async function loadChallengeComments(challengeId, page = 1) {
    const list = activeChallengeView?.querySelector('#challengeCommentsList');
    if (!list) return;

    if (page === 1) { list.innerHTML = '<div class="spinner"></div>'; currentChallengeCommentPage = 1; }
    else { const b = list.querySelector('.load-more-comments-btn'); if (b) b.innerText = 'Yükleniyor...'; }
    ...
    try {
        ...
        if (data.comments?.length > 0) {
            ...
            if (data.has_next) {
                ...
                btn.className = 'load-more-comments-btn';
                btn.innerText = 'Daha eski yorumlar';
                btn.addEventListener('click', () => loadChallengeComments(challengeId, ++currentChallengeCommentPage));
                list.appendChild(btn);
            }
        } else if (page === 1) {
...
function submitChallengeComment() {
    ...
    const txt = activeChallengeView.querySelector('#challengeCommentInput').value.trim();
    if (!txt) return showCustomAlert("Yorum yazın.", "error");
    if (txt.length > 300) return showCustomAlert("Çok uzun.", "error");

    const btn = activeChallengeView.querySelector('#submitChallengeCommentBtn');
    btn.disabled = true;

    apiRequest('/api/challenge-comment', 'POST', { challenge_id: currentChallengeId, comment: txt })
        .then(data => {
            showCustomAlert("Yorum eklendi!");
            activeChallengeView.querySelector('#challengeCommentInput').value = '';
            const list = activeChallengeView.querySelector('#challengeCommentsList');
