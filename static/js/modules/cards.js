...
export function createCardElement(item, isModalMode) {
    ...
    if (cCount === 0) hint.classList.add('is-empty');

    const hintIcon = document.createElementNS('http://www.w3.org/2000/svg', 'svg');
    hintIcon.setAttribute('class', 'click-hint-icon');
    ...
    hintIconPath.setAttribute('d', 'M21 11.5a8.38 8.38 0 0 1-.9 3.8 8.5 8.5 0 0 1-7.6 4.7 8.38 8.38 0 0 1-3.8-.9L3 21l1.9-5.7a8.38 8.38 0 0 1-.9-3.8 8.5 8.5 0 0 1 4.7-7.6 8.38 8.38 0 0 1 3.8-.9h.5a8.48 8.48 0 0 1 8 8v.5z');
    hintIcon.appendChild(hintIconPath);
    hint.appendChild(hintIcon);

    const hintLabel = document.createElement('span');
    hintLabel.className = 'click-hint-label';
    hintLabel.textContent = 'Yorumlar';
    hint.appendChild(hintLabel);

    const hintCount = document.createElement('span');
    hintCount.className = 'comment-count';
    hintCount.textContent = cCount;
    hint.appendChild(hintCount);

    const hintArrow = document.createElement('span');
    hintArrow.className = 'click-hint-arrow';
    hintArrow.textContent = '→';
    hint.appendChild(hintArrow);

    hint.setAttribute('aria-label', `${cCount} yorum`);
